...
try {
    ...
    if ($deployment) {
        ...
        if ($available -ge $desired -and $ready -ge $desired) {
            ...
            Add-Diagnosis "Deployment replicas are not available. Fix pod scheduling, image, or application startup before chasing service networking."
        }
    }
    ...
    try {
        ...
        foreach ($pod in $pods) {
            ...
            foreach ($containerStatus in @($pod.status.containerStatuses + $pod.status.initContainerStatuses)) {
                if ($containerStatus.state.waiting.reason) {
                    $badReasons += "$($pod.metadata.name):$($containerStatus.name)=$($containerStatus.state.waiting.reason)"
                }
                if ($containerStatus.lastState.terminated.reason) {
                    $badReasons += "$($pod.metadata.name):$($containerStatus.name)=last:$($containerStatus.lastState.terminated.reason)"
                }
            }
        ...
        if ($pods.Count -gt 0) {
            if ($running -eq $pods.Count) {
                ...
                Add-Diagnosis "Some pods are not Running. This is probably application, scheduling, or image pull health before networking."
            }
            ...
            if ($readyCount -eq $pods.Count) {
                ...
                Add-Diagnosis "Pods are not Ready. Services normally avoid routing to unready pods, so fix readiness/application health first."
            }
        }
        ...
        if ($badReasons.Count -gt 0) {
            ...
            Add-Diagnosis "Container waiting/terminated states were detected. Check image pulls, crashes, commands, probes, and app logs."
    ...
    if ($service) {
        ...
        if ($ExpectedPort -gt 0) {
            ...
            if ($matchingPort.Count -gt 0) {
                ...
                Add-Diagnosis "The service does not expose the expected port. Check service port/targetPort definitions."
            }
        }
    } else {
    ...
    if ($service) {
        try {
            ...
            if ($endpointIps.Count -eq 0) {
                ...
                Add-Diagnosis "Pods may be healthy but the service has no ready endpoints. Likely selector mismatch, readiness issue, or EndpointSlice controller problem."
            } else {
    ...
    if ($debugReady -and $podIps.Count -gt 0) {
        $urls = Get-ServiceUrls -Service $service
        foreach ($podIp in @($podIps | Sort-Object -Unique)) {
            ...
            if ($curl.ExitCode -eq 0) {
                ...
                Add-Diagnosis "Pod IP curl failed from inside the cluster. If pods are Ready, check NetworkPolicy, CNI/overlay networking, and whether the app listens on the expected port."
            }
        }

...
function Update-AutomationReport {
    param (
        [string]$ReportPath
    )
    ...
    $data | Export-Csv $ReportPath -NoTypeInformation
}
...
function Pkg-Apps {
    ...
    try {
        ...
        if (Test-Path "$ReleaseDir\$AppName-$($CurrentVersion.ToString())\SQL-Prior") {
            ...
            Centralized-Log "SQL-Prior did not exist in the previous version. Nothing to copy"
        }
        ...
        # Call Update-AutomationReport
        Update-AutomationReport -ReportPath "$Report\Report.csv"
    } catch {
